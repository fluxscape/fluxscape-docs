...
const { CustomMarkupPlugin } = require("./markdown-syntax");
...
export function includeMarkdownPlugin({ resolveFrom, resolveMdx } = {}) {
  return function transformer(tree, file) {
    return flatMap(tree, (node) => {
      ...
      if (includePath.match(/\.md(?:x)?$/)) {
        ...
        const processor = remark().use(CustomMarkupPlugin);
        // NOTE: Use our CustomMarkupPlugin plugin
